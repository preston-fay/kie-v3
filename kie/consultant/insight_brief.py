"""
Insight Brief Generator (STEP 3: CONSULTANT WOW FACTOR)
...
class InsightBriefGenerator:
    ...
    def __init__(self, project_root: Path):
        """
        Initialize Insight Brief Generator.
    ...
    def generate(self, evidence_ledger_id: str | None = None) -> dict[str, Any]:
        """
        Generate Insight Brief from existing artifacts.

        Args:
            evidence_ledger_id: Optional evidence ledger ID to link to
        ...
        if not insights_catalog_path.exists():
            return {
                "success": False,
                "message": "Cannot generate Insight Brief: insights_catalog.json not found",
                "missing_artifact": "outputs/insights_catalog.json",
                "recovery": [
                    "/analyze    # Run analysis first to generate insights",
                ],
            }
        ...
        artifact_hashes = self._get_artifact_hashes(evidence_ledger_id)
        ...
        brief_json_path = self.outputs_dir / "insight_brief.json"
    ...
    def _get_artifact_hashes(self, evidence_ledger_id: str | None) -> dict[str, str]:
        """Get artifact hashes from evidence ledger."""
        hashes = {}
        ...
        ledger_path = self.evidence_dir / f"{evidence_ledger_id}.yaml"
    ...
    def _generate_brief_content(
        ...
        """Generate markdown brief content."""
        lines = []
    ...
    def _generate_brief_data(
        ...
        """Generate structured JSON brief data."""
        return {
