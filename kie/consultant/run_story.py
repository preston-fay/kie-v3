"""
Run Story Generator (STEP 3: CONSULTANT WOW FACTOR)
...
class RunStoryGenerator:
    ...
    def __init__(self, project_root: Path):
        """
        Initialize Run Story Generator.
        ...
        self.evidence_dir = project_root / "project_state" / "evidence_ledger"

    def generate(self, since: str | None = None) -> dict[str, Any]:
        """
        Generate Run Story from recent commands.

        Args:
            since: Optional timestamp to start from (ISO format)

        Returns:
            Result dictionary with story path
        """
        # Collect evidence ledger entries
        entries = self._collect_ledger_entries(since)

        if not entries:
            return {
                "success": False,
                "message": "No evidence ledger entries found to generate story",
            }

        # Load artifacts
        spec = self._load_spec()
        insights_catalog = self._load_insights_catalog()
        insight_brief = self._load_insight_brief()
    ...
    def _collect_ledger_entries(self, since: str | None) -> list[dict[str, Any]]:
        """Collect evidence ledger entries."""
        if not self.evidence_dir.exists():
            return []

        entries = []
        for ledger_file in sorted(self.evidence_dir.glob("*.yaml")):
    ...
    def _load_spec(self) -> dict[str, Any] | None:
        """Load project spec."""
        spec_path = self.project_state_dir / "spec.yaml"
    ...
    def _generate_story_content(
        ...
        """Generate markdown story content."""
        lines = []
    ...
    def _generate_story_data(
        ...
        """Generate structured JSON story data."""
        return {
