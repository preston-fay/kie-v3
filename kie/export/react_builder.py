...
export function Dashboard() {{
  ...
  const renderVisualQualityBadge = (quality: string) => {{
    ...
    if (quality === 'internal_only') {{
      return (
        <span className="inline-block text-xs bg-red-500/20 text-red-400 px-2 py-1 rounded">
          Internal Only
        </span>
      );
    }}
            ...
            {{section.visuals.map((visual, idx) => (
              <div key={{idx}} className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>
                    {{visual.role}}: {{visual.transition_text}}
                  </span>
  ...
  return (
    <div className="min-h-screen bg-background p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="mb-2">{{manifest.project_name}}</h1>
          <p className="text-muted-foreground">{{manifest.objective}}</p>
        </div>
      </div>

      <Tabs defaultValue="main-story" className="space-y-6">
        <TabsList>
          <TabsTrigger value="main-story">Main Story</TabsTrigger>
          <TabsTrigger value="appendix">Appendix</TabsTrigger>
        </TabsList>

        <TabsContent value="main-story" className="space-y-6">
          {{mainStorySections.length > 0 ? (
            mainStorySections.map(renderSection)
          ) : (
            <Card className="p-6">
              <p className="text-muted-foreground">No main story sections available.</p>
            </Card>
          )}}
        </TabsContent>
        ...
        <TabsContent value="appendix" className="space-y-6">
          {{appendixSections.length > 0 ? (
            ...
            <Card className="p-6">
              <p className="text-muted-foreground">No appendix sections available.</p>
            </Card>
          )}}
        </TabsContent>
      </Tabs>
      ...
      <footer className="text-center text-sm text-muted-foreground py-8">
        <p>{{manifest.project_name}} | Generated with KIE v3 | Story Manifest Dashboard</p>
      </footer>
    </div>
  );
}}
