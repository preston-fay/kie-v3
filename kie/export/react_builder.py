...
'''
    ...
    def _generate_dashboard_component(self, src_dir: Path, data_path: Path):
        ...
        if self.column_mapping:
            ...
            categorical_cols[0] if len(categorical_cols) > 0 else 'entity'
            ...
            numeric_cols[2] if len(numeric_cols) > 2 else metric1_col
  ...
  return (
    <div className="min-h-screen bg-background p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="mb-2">{self.project_name}</h1>
          <p className="text-muted-foreground">{self.objective}</p>
        </div>
      </div>
