...
**ONLY PERMITTED:**
- ✅ KIE ChartFactory via CLI: All chart commands use the vendored runtime
...
Use the KIE CLI commands which internally use ChartFactory:
```bash
# ✅ CORRECT - Use KIE CLI for chart generation
PYTHONPATH=".kie/src" python3 -m kie.cli analyze  # Generates insights + charts
PYTHONPATH=".kie/src" python3 -m kie.cli build    # Builds full deliverable
```

**FORBIDDEN - Never write Python code with matplotlib:**
```python
# ❌ FORBIDDEN - Never use matplotlib in project scripts
...
- All visualizations MUST go through KIE CLI commands for KDS enforcement

**If user explicitly requests matplotlib:**
- Explain that KIE v3 uses Recharts for KDS compliance
- Offer to generate charts via KIE CLI commands instead
